...
//
// https://blockchaininfo.news/wp-json/wordpress-popular-posts/v1/popular-posts?order=desc&orderby=count&range=weekly
...
class RankingTableViewController: UITableViewController, IndicatorInfoProvider {

    let url = URL(string: "https://blockchaininfo.news/wp-json")!
    let urlPopularWeekly = URL(string: "https://blockchaininfo.news/wp-json/wordpress-popular-posts/v1/popular-posts?order=desc&orderby=count&range=weekly")!
    let urlPopularWeeklyTest = URL(string: "http://blockchaininfo.test/wp-json/wordpress-popular-posts/v1/popular-postvarrder=desc&orderby=count")!
    
    
    //ここがボタンのタイトルに利用されます
    var itemInfo: IndicatorInfo = "人気記事"
    ...
    */
    ...
    override func viewDidLoad() {
        ...
        print("hi")
        print(UserDefaults.standard.stringArray(forKey: "articleID"))
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        //urlPopularWeeklyは今週の人気記事のデータを人気順で返すURL。
        let task = URLSession.shared.dataTask(with: urlPopularWeekly) { (data, response, error) in
            do {
                //データをJSONにする
 ...
 */
    ...
    func fetchData() -> Promise<URL> {
        ...
        self.articleCategoryURL = nil
            ...
            let task = URLSession.shared.dataTask(with: self.urlPopularWeekly) { (data, response, error) in
                do {
                    //データをJSONにする
                     let json = try JSONSerialization.jsonObject(with: data!, options: JSONSerialization.ReadingOptions.allowFragments) as! NSArray
